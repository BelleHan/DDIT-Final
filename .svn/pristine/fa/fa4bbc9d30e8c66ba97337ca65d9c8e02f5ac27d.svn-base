...
public class LoginSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler{
	...
	private String savePath = "c:\\fran";
	private String saveFileName = "login_franUser_log.csv";
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		UserFran user = (UserFran) authentication.getDetails();
		
		FranVO loginFran = user.getFranVO();
...
//		MemberVO loginUser = service.getMember(username);
		...
		session.setAttribute("loginFran", loginFran);
		session.setMaxInactiveInterval(60*60);
		
		//log작성
		loginUserlogFile(loginFran, request);
		
		super.onAuthenticationSuccess(request, response, authentication);//여기서 redirect도 다해줘
		
	}

	private void loginUserlogFile(FranVO loginFran, HttpServletRequest request)throws IOException{
		//로그인 정보를 스트링으로 저장
		String tag ="[login:user]";
		String log = tag+loginFran.getFran_code()+","
