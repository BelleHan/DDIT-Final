			...
			<div class="card card-secondary card-tabs" style="overflow-x: hidden;" >
				<div class="card-header p-0 pt-1">
					<ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
						<li class="nav-item"><a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill" href="#custom-tabs-one-home" role="tab" aria-controls="custom-tabs-one-home" aria-selected="true">가맹점 안전/적정 재고량 설정</a></li>
					</ul>
				</div>
						...
						<tbody id="fran-stock-list">
		                    <c:forEach items="${franStockList }" var="stock">
								...
								<tr class="fran-stock" style="cursor: pointer;">
									...
									<td>${stock.fran_code }<input type="hidden" id="franCode${i }" value="${stock.fran_code }"></td>
									<td>${stock.fran_name }</td>
									<td>${stock.sup_goods_laca }</td>
									<td>${stock.sup_goods_smca }</td>
									<td>${stock.sup_goods_name }</td>
									<td>${stock.hq_goods_code }
									<input type="hidden" id="goodsCode${i }" value="${stock.hq_goods_code }">
									</td>
									...
									<td id="safe${i }"><fmt:formatNumber value="${stock.fran_safe_stock}" pattern="#,###" />
									</td>
									<td id="suit${i }"><fmt:formatNumber value="${stock.fran_suit_stock}" pattern="#,###" />
									</td>
									<td><button type="button" class="btn btn-warning" id="modifyBtn${i }" onclick="modifyGo('${i }')" style="margin-top: 4px; font-size: 3px; width: 50px;">수정</button>
									<button type="button" class="btn btn-primary" id="saveBtn${i }" onclick="saveGo('${i }');" style="font-size: 3px; margin-top: 4px; display:none;  width: 50px;">저 장</button>
									<button type="button" class="btn btn-primary" id="cancelBtn${i }" onclick="cancelGo('${i }')" style="margin-top: 4px; display:none; font-size: 3px;  width: 50px;">취소</button></td>
								</tr>
							</c:forEach>
		                 </tbody>
					</table>
...
function modifyGo(idx) {
	...
	$('#safe' + idx).append('<td style="position: relative; left: 10px;"><input type="text" id="fran_safe_stock'+idx+'" name="fran_safe_stock" class="form-control" style="width:100px;"></td>');
	$('#suit' + idx).append('<td style="position: relative; left: 10px;"><input type="text" id="fran_suit_stock'+idx+'" name="fran_suit_stock" class="form-control" style="width:100px;"></td>');
}
...
function saveGo(idx){
	var franCode = $("#franCode" + idx).val();
	var safeNum = $("#fran_safe_stock" + idx).val();
	var suitNum = $("#fran_suit_stock" + idx).val();
	var goodsCode = $("#goodsCode" + idx).val();
	
	
	
	data = {"fran_code" : franCode, "fran_safe_stock" : safeNum , "fran_suit_stock":suitNum, "hq_goods_code": goodsCode}
	...
	$.ajax({
		...
		, error:function(){
			if($("#fran_safe_stock" + idx).val() == ""){ 
				alert("안전재고를 입력해주세요"); $("#fran_safe_stock").focus(); return false; } 
...
<script type="text/x-handlerbars-template" id="fran-list-template">
	{{#each .}}  
			<tr class="fran" id="fran{{math @key '+' 1}}" onclick="javascript:franKey('{{fran_name}}');">
				<td>{{fran_code}}</td>
				<td>{{fran_name}}</td>
				<td>{{fran_owner}}</td>
