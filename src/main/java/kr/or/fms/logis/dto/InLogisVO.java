package kr.or.fms.logis.dto;

import java.util.Date;

public class InLogisVO {
	
	private String goods_pur_code;
	private String sup_name;
	private String sup_goods_code;
	private String sup_goods_name;
	private String logis_code;
	private Date in_date;
	private Date in_date_date;
	private int in_num;
	private int in_num_schedule;
	private String in_st;
	private String in_loca;
	private String wh_code;
	private Date in_date_schedule;
	private String in_name;
	private String in_code;
	public String getIn_code() {
		return in_code;
	}
	public void setIn_code(String in_code) {
		this.in_code = in_code;
	}
	public String getIn_name() {
		return in_name;
	}
	public void setIn_name(String in_name) {
		this.in_name = in_name;
	}
	public String getSup_name() {
		return sup_name;
	}
	public void setSup_name(String sup_name) {
		this.sup_name = sup_name;
	}
	public String getSup_goods_name() {
		return sup_goods_name;
	}
	public void setSup_goods_name(String sup_goods_name) {
		this.sup_goods_name = sup_goods_name;
	}
	public String getGoods_pur_code() {
		return goods_pur_code;
	}
	public void setGoods_pur_code(String goods_pur_code) {
		this.goods_pur_code = goods_pur_code;
	}
	public String getSup_goods_code() {
		return sup_goods_code;
	}
	public void setSup_goods_code(String sup_goods_code) {
		this.sup_goods_code = sup_goods_code;
	}
	public String getLogis_code() {
		return logis_code;
	}
	public void setLogis_code(String logis_code) {
		this.logis_code = logis_code;
	}
	public Date getIn_date() {
		return in_date;
	}
	public void setIn_date(Date in_date) {
		this.in_date = in_date;
	}
	public Date getIn_date_date() {
		return in_date_date;
	}
	public void setIn_date_date(Date in_date_date) {
		this.in_date_date = in_date_date;
	}
	public int getIn_num() {
		return in_num;
	}
	public void setIn_num(int in_num) {
		this.in_num = in_num;
	}
	public int getIn_num_schedule() {
		return in_num_schedule;
	}
	public void setIn_num_schedule(int in_num_schedule) {
		this.in_num_schedule = in_num_schedule;
	}
	public String getIn_st() {
		return in_st;
	}
	public void setIn_st(String in_st) {
		this.in_st = in_st;
	}
	public String getIn_loca() {
		return in_loca;
	}
	public void setIn_loca(String in_loca) {
		this.in_loca = in_loca;
	}
	public String getWh_code() {
		return wh_code;
	}
	public void setWh_code(String wh_code) {
		this.wh_code = wh_code;
	}
	public Date getIn_date_schedule() {
		return in_date_schedule;
	}
	public void setIn_date_schedule(Date in_date_schedule) {
		this.in_date_schedule = in_date_schedule;
	}
	
}
