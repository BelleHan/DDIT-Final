...
<body>
	...
	<div class="card" style="max-width : 1270px; margin-left : 5px; margin-bottom : 5px;">
		<div class="card card-secondary card-tabs" style="overflow-x: hidden; margin-bottom : 1px; margin-top : 3px;" >
			<div class="card-header p-0 pt-1">
				<ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
					<li class="nav-item"><a class="nav-link active" id="custom-tabs-one-home-tab" style="height: 2.3rem; padding-top: 13px;" data-toggle="pill" href="#custom-tabs-one-home" role="tab" aria-controls="custom-tabs-one-home" aria-selected="true">재고실사</a></li>
				</ul>
			</div>
		</div>
		...
		<div class="card col-6 float-left" style="max-width : 560px; margin-left: 3px; min-height: 38.4rem;">
	                ...
	                <table class="table table-hover" >
						...
						<tbody id="logis-stock-list">
 		                    <c:forEach items="${logisStockTakingList }" var="stock">
								...
								<tr class="logis-stock" id="taking-${i}" onclick="javascipt:taking_go('${stock.taking_code }');" style="cursor: pointer;">
									<td>${i }</td>
									<td id="tr-${i }">${stock.taking_code }</td>
		...
		<div class="card col-6  float-left" style="margin-left: 3px; min-width: 708px; min-height: 38.4rem; overflow-y: auto;">
...
function searchGoStockL(realpage, url){
	...
	end_date = dateArr[4] + dateArr[5] + (+dateArr[6]+1);
	data = {"fms_logis": fms_logis, "fms_wh": fms_wh, "page" : realpage, "start_date" : start_date, "end_date" : end_date};
	console.log(data);
	$.ajax({
		url : url
		, method : 'post'
